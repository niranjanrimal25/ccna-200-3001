...
@section('content')
    <h1 class="text-3xl font-bold text-slate-100">CCNA 200-301 Study Platform</h1>
                            ...
                            @foreach ($topic->lessons as $lesson)
                                <li>
                                    <a href="{{ route('lessons.show', [$topic, $lesson]) }}"
                                       class="group block rounded-lg border border-slate-800 bg-slate-900 p-4 transition-colors hover:border-blue-700 hover:bg-slate-800/60">
                                        <div class="font-medium text-slate-100 group-hover:text-blue-300">
                                            {{ $lesson->title }}
                                        </div>
                                        @if ($lesson->summary)
                                            <p class="mt-1 text-sm text-slate-400">{{ $lesson->summary }}</p>
                                        @endif
