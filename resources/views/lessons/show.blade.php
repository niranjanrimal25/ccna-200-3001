...
@section('content')
    ...
    <div class="mt-4 flex flex-wrap items-center gap-3">
        <span class="rounded-full border border-teal-500/40 bg-teal-500/15 px-3 py-1 text-xs font-bold uppercase tracking-wider text-teal-300">
            Day {{ $day }}
        </span>
        <span class="text-xs text-zinc-500">{{ $lesson->topic->title }}</span>
    </div>

    <h1 class="mt-3 text-3xl font-bold text-zinc-100">{{ $lesson->title }}</h1>

    @if ($lesson->summary)
        <p class="mt-3 text-zinc-400 leading-relaxed">{{ $lesson->summary }}</p>
    @endif

    @if ($lesson->source_ref)
        <p class="mt-2 text-xs text-zinc-600">Source: {{ $lesson->source_ref }}</p>
    @endif
    ...
    <div x-data="notesPdf" class="mt-5">
        ...
        <p class="mt-2 text-xs text-zinc-600">Handwritten notebook-style notes with diagrams, tables, and quiz review.</p>
    ...
    <div class="mt-8 space-y-8">
        @foreach ($lesson->sections as $section)
            <section id="section-{{ $section->id }}" class="scroll-mt-8">
                @if ($section->title)
                    <h2 class="mb-3 text-xl font-semibold text-zinc-200">{{ $section->title }}</h2>
                @endif
    ...
    <nav class="mt-12 flex items-center justify-between border-t border-zinc-800 pt-6">
        @if ($prev)
