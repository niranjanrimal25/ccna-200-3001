            ...
            <g transform="translate({{ $node['x'] }}, {{ $node['y'] }})">
                <rect x="-40" y="-34" width="80" height="56" rx="6" fill="none"
                    @if (in_array((string) $node['id'], $highlights, true))
                        stroke="#2dd4bf" stroke-width="2.5" stroke-dasharray="5 3"
                    @else
                ...
                <text text-anchor="middle" y="34" fill="#d4d4d8" font-size="13" font-weight="500">
                    {{ $node['label'] }}
                </text>
            </g>
        @endforeach
        ...
        <g class="pointer-events-none">
            @foreach ($packets as $packet)
                <g transform="translate({{ $packet['x'] }}, {{ $packet['y'] }})">
                    <circle r="6" fill="{{ $packet['color'] ?? '#2dd4bf' }}"/>
