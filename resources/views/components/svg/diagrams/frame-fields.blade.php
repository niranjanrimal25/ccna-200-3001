...
<div class="rounded-xl border border-stone-800 bg-stone-900/60 p-4">
    <div class="overflow-x-auto rounded-md">
        <div class="flex min-w-[600px]">
            @foreach ($content['fields'] as $field)
                ...
                @endphp
                <div class="shrink-0 border border-stone-700 p-3 text-center"
                     style="width: {{ $pct }}%; min-width: {{ max(64, $bytes * 14) }}px;">
                    <div class="text-xs font-semibold text-stone-200">{{ $field['name'] }}</div>
                    <div class="mt-1 text-[10px] text-stone-500">{{ $field['length'] }}</div>
                    @if (!empty($field['purpose']))
                        <div class="mt-1 text-[10px] leading-snug text-stone-500">{{ $field['purpose'] }}</div>
                    @endif
                    @if ($isTrailer)
    ...
    @if (! empty($content['note']))
        <p class="mt-3 text-xs text-stone-500">{{ $content['note'] }}</p>
    @endif
</div>
